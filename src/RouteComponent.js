...
export default class RouteComponent extends Component {
  ...
  onChange = (e, key) => {
    ...
    if (
      ...
      this.state.currentLat.length > 0 &&
      this.state.currentLong.length > 0
    ) {
      this.setState({
        enableSubmit: true,
      });
              ...
              <Col xs={4}>
                <div style={{ marginTop: "70px" }}>
                  {!this.state.enableSubmit && (
                    <Button className="submit-button-disable" variant="primary">
                      Submit
                    </Button>
                  )}
                  {this.state.enableSubmit && (
                    <Button
                      className="submit-button-enable"
